  ...
  <main>
    <section>
      <div class="row bg-brand text-white q-pa-sm">
        <div class="col-6">Region: {{ Western.region }}</div>
        <div class="col-6">Capital: {{ Western.capital }}</div>
      </div>
    </section>
    ...
    <section class="row q-col-gutter-sm q-pa-sm constituencies">
      <div v-for="(constituency, index) in constituencies()" :key="constituency.code" class="col-6 constituency"
        ...
        <router-link :to="`/constituencies/western/${constituency.constituency
          .toLowerCase()
          .replaceAll(' ', '-')}`">
...
import { Western } from "../../../data/constituency results/western";
const _all = Western.results;
...
const candidates = Western.results.map((item) => {
...
const _constituencies = Western.results.map((i) => i.constituency);
