  ...
  <main>
    <section>
      <div class="row bg-brand text-white q-pa-sm">
        <div class="col-6">Region: {{ WesternNorth.region }}</div>
        <div class="col-6">Capital: {{ WesternNorth.capital }}</div>
      </div>
    </section>
    ...
    <section class="row q-col-gutter-sm q-pa-sm constituencies">
      <div v-for="(constituency, index) in constituencies()" :key="constituency.code" class="col-6 constituency"
        ...
        <router-link :to="`/constituencies/western-north/${constituency.constituency
          .toLowerCase()
          .replaceAll(' ', '-')}`">
...
import { WesternNorth } from "../../../data/constituency results/westernNorth";
const _all = WesternNorth.results;
...
const candidates = WesternNorth.results.map((item) => {
...
const _constituencies = WesternNorth.results.map((i) => i.constituency);
