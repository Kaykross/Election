  ...
  <main>
    <section>
      <div class="row bg-brand text-white q-pa-sm">
        <div class="col-6">Region: {{ GreaterAccra.region }}</div>
        <div class="col-6">Capital: {{ GreaterAccra.capital }}</div>
      </div>
    </section>
    ...
    <section class="row q-col-gutter-sm q-pa-sm constituencies">
      <div v-for="(constituency, index) in constituencies()" :key="constituency.code" class="col-6 constituency"
        ...
        <router-link :to="`/constituencies/greater-accra/${constituency.constituency
          .toLowerCase()
          .replaceAll(' ', '-')}`">
...
import { GreaterAccra } from "../../../data/constituency results/greaterAccra";
const _all = GreaterAccra.results;
...
const candidates = GreaterAccra.results.map((item) => {
...
const _constituencies = GreaterAccra.results.map((i) => i.constituency);
