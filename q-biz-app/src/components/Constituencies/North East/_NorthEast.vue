  ...
  <main>
    <section>
      <div class="row bg-brand text-white q-pa-sm">
        <div class="col-6">Region: {{ NorthEast.region }}</div>
        <div class="col-6">Capital: {{ NorthEast.capital }}</div>
      </div>
    </section>
    ...
    <section class="row q-col-gutter-sm q-pa-sm constituencies">
      <div v-for="(constituency, index) in constituencies()" :key="constituency.code" class="col-6 constituency"
        ...
        <router-link :to="`/constituencies/north-east/${constituency.constituency
          .toLowerCase()
          .replaceAll(' ', '-')}`">
...
import { NorthEast } from "../../../data/constituency results/northEast";
const _all = NorthEast.results;
...
const candidates = NorthEast.results.map((item) => {
...
const _constituencies = NorthEast.results.map((i) => i.constituency);
