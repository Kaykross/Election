  ...
  <main>
    <section>
      <div class="row bg-brand text-white q-pa-sm">
        <div class="col-6">Region: {{ Eastern.region }}</div>
        <div class="col-6">Capital: {{ Eastern.capital }}</div>
      </div>
    </section>
    ...
    <section class="row q-col-gutter-sm q-pa-sm constituencies">
      <div v-for="(constituency, index) in constituencies()" :key="constituency.code" class="col-6 constituency"
        ...
        <router-link :to="`/constituencies/eastern/${constituency.constituency
          .toLowerCase()
          .replaceAll(' ', '-')}`">
...
import { Eastern } from "../../../data/constituency results/eastern";
const _all = Eastern.results;
...
const candidates = Eastern.results.map((item) => {
...
const _constituencies = Eastern.results.map((i) => i.constituency);
