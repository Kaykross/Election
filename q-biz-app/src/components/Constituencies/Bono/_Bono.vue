  ...
  <main>
    <section>
      <div class="row bg-brand text-white q-pa-sm">
        <div class="col-6">Region: {{ Bono.region }}</div>
        <div class="col-6">Capital: {{ Bono.capital }}</div>
      </div>
    </section>
    ...
    <section class="row q-col-gutter-sm q-pa-sm constituencies">
      <div v-for="(constituency, index) in constituencies()" :key="constituency.code" class="col-6 constituency"
        ...
        <router-link :to="`/constituencies/bono/${constituency.constituency
          .toLowerCase()
          .replaceAll(' ', '-')}`">
...
import { Bono } from "../../../data/constituency results/bono";

const _all = Bono.results;
...
const candidates = Bono.results.map((item) => {
...
const _constituencies = Bono.results.map((i) => i.constituency);
