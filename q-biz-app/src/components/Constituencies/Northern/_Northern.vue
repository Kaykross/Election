  ...
  <main>
    <section>
      <div class="row bg-brand text-white q-pa-sm">
        <div class="col-6">Region: {{ Northern.region }}</div>
        <div class="col-6">Capital: {{ Northern.capital }}</div>
      </div>
    </section>
    ...
    <section class="row q-col-gutter-sm q-pa-sm constituencies">
      <div v-for="(constituency, index) in constituencies()" :key="constituency.code" class="col-6 constituency"
        ...
        <router-link :to="`/constituencies/northern/${constituency.constituency
          .toLowerCase()
          .replaceAll(' ', '-')}`">
...
import { Northern } from "../../../data/constituency results/northern";
const _all = Northern.results;
...
const candidates = Northern.results.map((item) => {
...
const _constituencies = Northern.results.map((i) => i.constituency);
