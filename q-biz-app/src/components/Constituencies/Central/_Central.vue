  ...
  <main>
    <section>
      <div class="row bg-brand text-white q-pa-sm">
        <div class="col-6">Region: {{ Central.region }}</div>
        <div class="col-6">Capital: {{ Central.capital }}</div>
      </div>
    </section>
    ...
    <section class="row q-col-gutter-sm q-pa-sm constituencies">
      <div v-for="(constituency, index) in constituencies()" :key="constituency.code" class="col-6 constituency"
        ...
        <router-link :to="`/constituencies/central/${constituency.constituency
          .toLowerCase()
          .replaceAll(' ', '-')}`">
...
import { Central } from "../../../data/constituency results/central";
const _all = Central.results;
...
const candidates = Central.results.map((item) => {
...
const _constituencies = Central.results.map((i) => i.constituency);
