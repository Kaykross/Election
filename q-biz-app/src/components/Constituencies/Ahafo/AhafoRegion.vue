    ...
    <section>
      <div>
        <q-card class="bg-milk-green">
          ...
          <q-card-section>
            <q-table :title="ConstituencyData[0].constituency + ` Presidential Results`
              " :rows="rows" :columns="columns" row-key="name" class="constituency-table" :pagination="pagination"
                ...
                <q-tr :props="props">
                  <q-td key="candidate" :props="props" class="bl-ndc">
                    <div style="display: flex; justify-content: space-between">
                      ...
                      <div>
                        <q-img :src="props.row.profile" spinner-color="primary" spinner-size="82px" sizes="20px"
                  ...
                  <q-td key="votes" :props="props">
                    <div style="display: flex; justify-content: space-between">
                      ...
                      <q-icon :color="props.row.votes == maxVote ? 'green' : 'red'" :name="props.row.votes == maxVote
                        ? 'arrow_upward'
                        : 'arrow_downward'
                        " />
                    </div>
          ...
          <q-card-section>
            <q-table :title="ConstituencyData[0].constituency + ` Parliamentary Results`
              " :rows="rowsParl" :columns="columns" row-key="name" class="parliamentary-table" :pagination="pagination"
                ...
                <q-tr :props="props">
                  ...
                  <q-td key="votes" :props="props">
                    <div style="display: flex; justify-content: space-between">
                      ...
                      <q-icon :color="props.row.votes == maxParl ? 'green' : 'red'" :name="props.row.votes == maxParl
                        ? 'arrow_upward'
                        : 'arrow_downward'
                        " />
                    </div>
                  ...
                  <q-td key="percent" :props="props">
                    <div style="display: flex; justify-content: space-between">
                      <q-badge class="q-pa-sm" color="primary" text-color="white" :label="props.row.percent + ' %'" />
...
const c_name = Route.params.name.toString().replace("-", " ");
const condition = Ahafo.results
  .map((i) => i.constituency.toLowerCase())
  .filter((i) => i == c_name);

condition.length ? null : Router.push("/error/invalid-constituency-name");
const ConstituencyName = condition.length ? ref(c_name) : ref("asunafo south");

// console.log(Ahafo);

const ConstituencyData = constituency(Ahafo, ConstituencyName.value);
