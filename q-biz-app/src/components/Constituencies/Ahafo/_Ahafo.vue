  ...
  <main>
    ...
    <section class="row q-col-gutter-sm q-pa-sm constituencies">
      <div v-for="(constituency, index) in constituencies()" :key="constituency.code" class="col-6 constituency"
        ...
        <router-link :to="`/constituencies/ahafo/${constituency.constituency
          .toLowerCase()
          .replaceAll(' ', '-')}`">
...
const candidates = Ahafo.results.map((item) => {
...
const _constituencies = Ahafo.results.map((i) => i.constituency);
