  ...
  <main>
    <section>
      <div class="row bg-brand text-white q-pa-sm">
        <div class="col-6">Region: {{ UpperWest.region }}</div>
        <div class="col-6">Capital: {{ UpperWest.capital }}</div>
      </div>
    </section>
    ...
    <section class="row q-col-gutter-sm q-pa-sm constituencies">
      <div v-for="(constituency, index) in constituencies()" :key="constituency.code" class="col-6 constituency"
        ...
        <router-link :to="`/constituencies/upper-west/${constituency.constituency
          .toLowerCase()
          .replaceAll(' ', '-')}`">
...
import { UpperWest } from "../../../data/constituency results/upperWest";
const _all = UpperWest.results;
...
const candidates = UpperWest.results.map((item) => {
...
const _constituencies = UpperWest.results.map((i) => i.constituency);
