  ...
  <main>
    <section>
      <div class="row bg-brand text-white q-pa-sm">
        <div class="col-6">Region: {{ Savannah.region }}</div>
        <div class="col-6">Capital: {{ Savannah.capital }}</div>
      </div>
    </section>
    ...
    <section class="row q-col-gutter-sm q-pa-sm constituencies">
      <div v-for="(constituency, index) in constituencies()" :key="constituency.code" class="col-6 constituency"
        ...
        <router-link :to="`/constituencies/savannah/${constituency.constituency
          .toLowerCase()
          .replaceAll(' ', '-')}`">
...
import { Savannah } from "../../../data/constituency results/savannah";
const _all = Savannah.results;
...
const candidates = Savannah.results.map((item) => {
...
const _constituencies = Savannah.results.map((i) => i.constituency);
