  ...
  <main>
    <section>
      <div class="row bg-brand text-white q-pa-sm">
        <div class="col-6">Region: {{ Oti.region }}</div>
        <div class="col-6">Capital: {{ Oti.capital }}</div>
      </div>
    </section>
    ...
    <section class="row q-col-gutter-sm q-pa-sm constituencies">
      <div v-for="(constituency, index) in constituencies()" :key="constituency.code" class="col-6 constituency"
        ...
        <router-link :to="`/constituencies/oti/${constituency.constituency
          .toLowerCase()
          .replaceAll(' ', '-')}`">
...
import { Oti } from "../../../data/constituency results/oti";
const _all = Oti.results;
...
const candidates = Oti.results.map((item) => {
...
const _constituencies = Oti.results.map((i) => i.constituency);
