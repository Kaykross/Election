  ...
  <main>
    <section>
      <div class="row bg-brand text-white q-pa-sm">
        <div class="col-6">Region: {{ Volta.region }}</div>
        <div class="col-6">Capital: {{ Volta.capital }}</div>
      </div>
    </section>
    ...
    <section class="row q-col-gutter-sm q-pa-sm constituencies">
      <div v-for="(constituency, index) in constituencies()" :key="constituency.code" class="col-6 constituency"
        ...
        <router-link :to="`/constituencies/volta/${constituency.constituency
          .toLowerCase()
          .replaceAll(' ', '-')}`">
...
import { Volta } from "../../../data/constituency results/volta";
const _all = Volta.results;
...
const candidates = Volta.results.map((item) => {
...
const _constituencies = Volta.results.map((i) => i.constituency);
